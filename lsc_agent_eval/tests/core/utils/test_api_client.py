...
class TestAgentHttpClient:
    ...
    def test_streaming_query_agent_not_initialized(self):
        """Test streaming agent query when client is not initialized."""
        client = AgentHttpClient("http://localhost:8080")
        client.client = None  # Uninitialized client

        api_input = {"query": "Test query", "provider": "openai", "model": "gpt-4"}

        with pytest.raises(AgentAPIError, match="HTTP client not initialized"):
            client.streaming_query_agent(api_input)
